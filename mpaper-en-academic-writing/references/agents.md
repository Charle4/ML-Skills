...
Assemble the Codex prompt from Blocks A + B + C, plus the review checklist below. Use `approval-policy: "never"` and `model_reasoning_effort: xhigh`. Omit `sandbox` so Codex can edit the draft file directly.
...
After applying verified Codex fixes, call Gemini to polish the draft. Gemini starts from the post-Codex-review draft, not the original. Assemble the Gemini prompt from Blocks A + B + D.
...
### Persona Constraint (use `system` parameter — Gemini specifically needs this)

Pass the persona constraint via the `system` parameter (separate from `prompt`), so Gemini applies it as a system-level instruction:
...
```
1. mcp__gemini-review__review_start (system: <persona>, prompt: <task>) → returns jobId
...
Follow-up uses `mcp__gemini-review__review_reply_start` with the `threadId`. The `system` parameter is also available on `review_reply_start`.
...
### Block C — Minimal Invasive Editing
...
### Block E — Draft Writing from Outline or Notes
...
### Block F — Literature Review / Related Work
...
| Task | Blocks |
|:-----|:-------|
| Coordinated Stage 2 (Codex logic review) | A + B + C + review checklist from Stage 2 |
| Coordinated Stage 3 (Gemini language polish) | A + B + D |
| Single-agent deep polish | A + B + C + D |
| Section drafting | A + B + E |
| Related-work synthesis | A + B + F |
| Final proofreading | A + B + C |
...
7. **Judge versions by evidence legibility, not by which one the agent prefers.** A cold-context reader's verdict moves with claim stance, evidence emphasis, and scope framing even when the reported content is identical, and asking that reader for a stricter review makes it harsher rather than insensitive to those cues. So an agent preferring your revision is not evidence that the revision is more sound. Compare the two versions on whether the comparison, condition, and margin became easier to find, and settle the rest yourself with the source in hand.
