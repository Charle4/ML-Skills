...
- 处理完整引言时，保留“问题背景—具体困难—已有工作缺口—本文判断—贡献”的连续收束。各阶段按信息量使用一个或多个段落，不为追求简短压成一两段生硬跳转，也不机械规定段数。
- 数学符号在首次出现前后定义。相关公式围绕同一推理目的成组组织，组前交代用途，组后解释所得判断；不形成“短句后紧跟孤立公式”的碎片化版面。
- 伪代码只用于必须展示非平凡执行顺序、循环条件或复现细节的过程。公式和正文已经说明清楚时，不为每个模块重复设置算法块。
- 项目符号只承载真正并列、可独立扫描的内容。因果、递进、依赖和时间承接关系写成连贯段落或明确的有序步骤。
- 每个模块使用一个简洁规范名称及必要缩写，首次定义后全文一致。不反复叠加“鲁棒、自适应、理论化、高效”等属性，不用加粗强化模块存在感。
- 避免装饰性分号和破折号，减少句中机械插入“因此”“由此”“进而”。“优雅地”“理论上有效地”等副词只有在明确指向可核验的理论结论时才保留。
- 方法部分说明机制、公式与实现。baseline 的定位放在相关工作，性能比较放在实验部分；定义方法所必需的简短对照可以保留。
- 局部修改保持局部层级。用户要求补充某个约束、模块或场景时，不把该需求升级为全文动机、主要贡献或结论，除非原稿与证据已经赋予其中心地位。
- 引用标记与其支撑的判断保持绑定。未检查原文时，不依据标题、摘要片段、相邻引用或他人综述推断贡献、结果或局限；无法确认时标记待核，不移动引用去支撑改写后的新主张。
...
- "为了解决这一痛点"可改为"针对上述问题"。
- "展现了令人惊叹的能力"通常直接删除评价；只有原文提供数据时，改写为具体指标及比较范围。
- "效果变好了"可改为"性能有所提升""模型在该指标上取得更优结果"。
- "不管是 A 还是 B"可改为"无论 A 抑或 B"。
- "我们发现"仅在证据对应时改为"实验结果表明""结果显示"或"分析表明"；不得用证据性主语包装未经验证的判断。
...
- 中文正文使用中文全角标点：，。；：""（）。
- 引号统一使用弯引号""；确有语义需要时使用破折号"—"，不把破折号作为行文节奏装饰，也不使用直引号、半角连字符、双连字符或其他 dash 变体替代。
...
编辑完成后，在对话回复中简要说明修改位置、处理范围，并按任务模式给出说明：

- 重构写作：说明重构思路，例如提取中心句、合并冗余描述、调整叙述语序、将列表整合为连贯段落。
- 克制润色：说明修改点，例如修复指代不明、去除口语表达、压缩欧化长句、统一术语。原文无需修改时，给出正向评价，如"原文逻辑清晰，表达规范，符合出版要求，建议保留。"
- 去 AI 味：简要列举处理了哪些空泛渲染表达、翻译腔句式、机械连接或长定语结构。原文已足够自然时，输出"原文表达严谨自然，无明显 AI 痕迹，建议保留。"
- 撰写辅助：说明段落的组织逻辑和技术要点的展开方式。
...
11. 完整引言是否保留自然的动机链；符号是否先定义，公式是否成组并有解释。
12. 伪代码、项目符号和强调格式是否均有结构必要性；递进关系是否被误写成平行分项。
13. 模块名称和缩写是否简洁一致；baseline 比较是否位于相关工作或实验部分。
14. 局部修改是否仍处于局部层级，未改变全文主线和贡献重心。
15. 每条引用是否仍支撑其相邻判断，无法核验的对应关系是否已标记待核。
