...
- Strategist returns per-HP observations and planned-queue candidates.
...
| 7 | **Self-declaring exhaustion, convergence, or plateau** — concluding from your own reading of results that the search space is exhausted, a ceiling has been reached, or that further tuning is futile, then stopping or skipping Strategist on that basis. Common disguises: "this is circular", "session is naturally concluding", "Strategist will just return 0 anyway", "the session has run its course", "search space is exhausted" | You lack the full search-history perspective; such judgments are highly error-prone. Only the Strategist has authority to declare exhaustion. Predicting that Strategist will return 0 and acting on that prediction is still self-evaluation — you cannot know what Strategist will decide until it actually decides. | Never self-evaluate plateau, convergence, or exhaustion. When the planned queue is insufficient, run the Strategist transaction. Only when Strategist returns 0 candidates and explicitly declares exhaustion should you consider stopping — and even then a second independent confirmer must agree. The script owns this: `aet.py strategist-return` sets the handshake, and the next `aet.py strategist-begin` forces a fresh independent confirmer (never a resume). See Default Stopping Rules. **Signal**: if you find yourself about to write "this direction is exhausted", "a ceiling has been reached", "this is circular", "Strategist will return 0 anyway", or "the session is naturally concluding" — that sentence is the trigger to run the transaction, not to stop. |
| 8 | **Skipping Strategist because it was "recently called" or "has no new data"** — reasoning that calling Strategist is pointless because it just ran, or because the pending set is empty, so there is nothing new for it to analyze | You do not decide whether to call the Strategist — `loop-state` decides. It compares an evidence hash to detect whether session state actually changed; when nothing changed, it routes a wait. Your job is to run `loop-state` and follow its `YOU` block, not to second-guess it. | Always run `aet.py loop-state` and follow its `YOU`. If it routes a Strategist call, run it. If it routes a wait, wait. **Signal**: if you find yourself thinking "Strategist was just called so I'll skip it" — run `loop-state` and follow its output instead. |
| 9 | **Skipping `loop-state` on keepalive invocations that found no new completions** — running `aet.py status`, seeing no change since last check, and concluding "nothing to do this invocation, continue waiting" without running `aet.py loop-state` | The loop-state check is unconditional and independent of whether new completions occurred. `aet.py status` showing no change means step (1) has no action; it does NOT mean the loop-state routing can be skipped. `loop-state` itself handles state-unchanged detection and will route a wait when appropriate. | After `aet.py status` shows no new completions: still run `aet.py loop-state` and follow its `YOU`. **Signal**: if you find yourself writing "no new completions, continue waiting" — that sentence is the trigger to run `loop-state`, not to stop. |
...
| 11 | **Closing a Strategist transaction with `--candidates-count 0` without actually running the Strategist** — running `strategist-begin`, then immediately calling `strategist-return --candidates-count 0` without making the printed tool call (SendMessage/Agent). Common disguises: "Quick close", "Close with 0", "I know the strategist will return 0", "let me just close the transaction" | The transaction exists to get an independent assessment from the Strategist. Bypassing Beat 2 (the actual subagent call) and fabricating a zero-candidate result is data falsification — it poisons the exhaustion handshake with a fake signal. Two fake zeros trigger CONFIRMED_EXHAUSTION for a search that was never actually evaluated. | Always execute all three beats: `strategist-begin` → the printed subagent call (SendMessage or Agent) → wait for the subagent to return → read its output → `strategist-return` with the actual candidate count. If the subagent truly returns 0 candidates, that is legitimate; but YOU deciding it will return 0 and skipping the call is not. |
| 12 | **Assuming a tool is unavailable because its schema is not visible** — concluding "the tool schema wasn't loaded" or "SendMessage/CronCreate is not in my tool list" and falling back to skipping the call or finding a workaround | All tools referenced in this skill (`SendMessage`, `CronCreate`, `CronDelete`, `CronList`, `Agent`) always work. Just output the tool call. "Schema not loaded" is not a real error and not a valid reason to skip any call — it is an excuse. Field-tested: every time an agent was forced to call a tool despite claiming it was unavailable, the call succeeded. | Output the tool call directly. Do not check whether it is "available" first. The ONLY signal a tool call failed is the call itself returning an error. |
| 13 | **Priming the Strategist prompt with stop/exhaustion language** — adding "session stop rule in effect", "this is the final batch", "search space nearly exhausted", or echoing plateau/ceiling language from `session.md` into the Strategist prompt | The Strategist must form its own independent conclusion from the raw evidence. Priming its prompt with your assessment of exhaustion or finality biases it toward returning 0 candidates, poisoning the exhaustion handshake. This is different from #7 (self-declaring exhaustion): here you delegate the declaration but rig the input. | Use the standard neutral prompt template from subagents.md with raw field values only. The `runs_since_last_strategist` line and `current_best` are locators, not interpretations. Do not add adjectives ("final", "last", "critical"), conclusions, or language from the Stop/Continue Rule section. |
| 14 | **Adding your own analysis or judgment to progress updates** — appending conclusions like "最终批次", "最后的解决方案", "this is the make-or-break run", "session in terminal phase" after executing `loop-state`'s instructions | Your role is orchestration, not analysis. Labeling runs as "final", "critical", or "make-or-break" is implicit self-evaluation of search state — the Strategist owns that judgment. These labels also prime your own future decisions toward premature stopping. | After executing all `loop-state` instructions, report only the factual state that `loop-state` printed: slot counts, planned count, and what you just launched. No adjectives, no predictions, no finality language. |
...
3. Determine terminal status: `finished`, `failed`, `inconclusive`, or `infra_failed`. The question is whether the failure says anything about the parameters: the run's own settings failing is `failed`, the environment failing is `infra_failed`. Classify against [Separating `failed` from `infra_failed`](references/workflow.md#separating-failed-from-infra_failed) — filing an infrastructure crash as `failed` fabricates a forbidden region and makes the Strategist avoid parameters that were never tested
4. Call `aet.py record --status <status> --run-id <id> [--primary-metric <v> --metric-name <n> --metrics '<json>'] [--annotation '<note>']`. `aet.py record` writes `results.csv` and adds the terminal run to the pending set in `loop_state.json`. `infra_failed` instead clones the candidate back into the planned queue and stays out of the pending set.
...
7. When loop-state routes a Strategist call (`projected_ready_after_launch < total_capacity`, where `projected_ready_after_launch = planned_count - min(free_slots, planned_count)`), run the **Strategist transaction** — three beats, every time (full detail in [references/subagents.md](references/subagents.md)):
   - **begin**: `aet.py strategist-begin` snapshots pending runs and finished evidence versions; computes the branch (fresh / resume / fresh confirmer); opens the call; and prints the exact spawn/resume tool call + payload. It refuses if a call is already open.
   - **tool_use**: make the printed call (Claude Code `Agent`/`SendMessage`; Codex `spawn_agent` or `send_message`/`followup_task`). For Claude Code resume, output the `SendMessage` tool call directly — it always works; do NOT skip it for any reason (anti-pattern #12). Between the two you may keep recording other completions; `record` adds them to pending without disturbing the open call.
   - **read output**: when the Strategist returns, read its full response — candidates, observations, rules, stop update, queue edits. Count the actual candidates. This count is `K` for the next beat. Skipping this and closing with fabricated `K=0` is anti-pattern #11.
   - **return**: `aet.py strategist-return --call-id C --candidates-count K [--agent-id A] [--observations-present] [--reusable-rules-present] [--queue-edits-present] [--stop-update-present]` clears version-matched snapshots, applies the handshake, and prints the `YOU` obligations. `K` must be the actual count from the Strategist's response. Pass `--agent-id` for every fresh invocation, including a confirmer.
   The script owns fresh-vs-resume-vs-confirmer, the exhaustion handshake, quiescence, and state-unchanged detection — do not hand-derive any of it. Agent-side suppression conditions: explicit user stop, target cleanly met, budget consumed, or permission unavailable. A call already open is script-enforced (`strategist-begin` refuses a second). `loop-state` also suppresses when session state is unchanged since the last Strategist call — follow its `YOU` block.
...
`planned` (registered candidate, not yet assigned a GPU) → `created` (GPU/output_dir assigned, not yet launched) → `running` → `finished` / `failed` / `inconclusive` / `infra_failed` / `superseded`. A `planned` candidate invalidated before launch goes `planned → dropped`. `infra_failed` also re-enters the queue: the candidate is cloned back to `planned` under a new run_id, with the queue_id carrying the attempt count (`Q7` → `Q7r1`).
...
| ------- | ---- | --------- |
| `projected_ready_after_launch < total_capacity` under the stored GPU policy | Strategist | Run the three-beat transaction: `aet.py strategist-begin` → printed tool call → `aet.py strategist-return`. Pass the id for every fresh invocation. Then follow `loop-state`: apply session/queue updates and launch planned rows. |

Agent-side suppression conditions: explicit user stop, explicit numeric target cleanly met with evidence, explicit run/wall-clock budget consumed, required permission/resource unavailable. Plateau and exhaustion are never self-evaluatable — Strategist must declare them. `loop-state` handles state-unchanged detection automatically — always run it and follow its `YOU`; do not self-evaluate whether calling Strategist is "worth it".
...
- `loop_state.json`: script-owned. Fields: pending run set, Strategist identity/open call, exhaustion handshake, agent history, and `last_evidence_hash` (state-unchanged detection). Read it with `aet.py loop-state`; change it only through AET helper commands. `loop-state` routes "wait" instead of re-routing Strategist when the session state has not changed since the last call.
...
- Treat `failed` and `inconclusive` runs as data. Record the failure pattern and avoid repeating it.
...
- Strategist calling is decided by `loop-state`: when `projected_ready_after_launch < total_capacity` and session state has changed since the last call, it routes a Strategist transaction. When session state is unchanged, it routes a wait. `aet.py strategist-begin` chooses blocking vs background and fresh vs resume vs confirmer. When a Strategist returns zero candidates the queue stays empty, so every subsequent trigger (after new evidence arrives) keeps running the transaction with the standard neutral prompt and complete session state.
...
7. Record results inline on completion:
   ...
   - Determine terminal status: `finished`, `failed`, `inconclusive`, or `infra_failed` (a failure the parameters had no part in — it re-queues the candidate and is kept out of the evidence set).
...
8. Update project records and continue:
   - Run `aet.py loop-state` and follow its `YOU` — the routing hub after every record. When session state is unchanged since the last Strategist call (no new completions, no slot changes), it prints a `wait` instead of re-routing the Strategist.
...
Run every `aet.py` command as a direct shell command (Claude Code: `Bash(python SKILL_DIR/scripts/aet.py <subcommand> ...)`; Codex: `exec_command`). The three-zone stdout (OK/STATE/YOU) is your instruction source — it must flow into the tool result untruncated. Use `scripts/aet.py` as the durable state and safety helper for the tuning loop:
...
- `record`: update a run whenever status or results change. Required: `--run-id`, `--status created|running|finished|failed|inconclusive|infra_failed|superseded`. Optional: `--session` or `--project-root`, `--primary-metric`, `--metric-name`, `--metrics` (JSON string or JSON file), `--gpu-id`, `--output-dir`, `--log-path`, `--annotation`, `--force`. Use `running` to record `start_time`; terminal statuses record `end_time` and add the run to the pending-run set. `infra_failed` records `end_time`, clones the candidate back into the planned queue under a new run_id, and adds nothing to the pending set. A terminal status is refused if the run's process is still alive — kill it first, or pass `--force` to override.
...
- `loop-state`: the decision panel — call each cycle and each keepalive tick. Counts the planned queue from `results.csv`. Optional: `--session`/`--project-root`, `--runtime` (override only). Prints OK/STATE plus a routed `YOU`/NEXT with specific run_id → GPU pairings, Strategist routing, or wait. When session state is unchanged since the last Strategist call (same terminal-run versions, planned/running counts, and free slots), it prints a `wait` instead of re-routing the Strategist — re-calling could not produce new information.
- `strategist-begin`: open a Strategist transaction (Beat 1). Optional: `--session`/`--project-root`, `--runtime` (override only). Snapshots every finished run's evidence version, computes fresh/resume/confirmer + blocking/background, opens `active_strategist_call`, and prints the exact spawn/resume tool call + payload. Refuses if a call is open.
- `strategist-return`: close the transaction (Beat 3). Required: `--call-id C`, non-negative `--candidates-count K`. Every fresh invocation also requires `--agent-id`. Clears the version-matched pending run snapshot, applies the exhaustion handshake, and stores an evidence hash for state-unchanged detection. `--resume-failed` is valid only for a resume call with `K=0`, no id, and no presence flags; it consumes no snapshot and preserves the evidence hash (no Strategist analyzed the evidence).
...
- `status` / `summarize`: print session path, objective, run counts, status counts, and current best.
...
- Subagent `experiment-strategist`: returns evidence-linked observations, reusable rules, the next candidate queue, stop/continue updates, queue edits, and escape/confirmation need. On the initial call it plans from `session.md` without completed runs. Invoke it inside the three-beat transaction described in [references/subagents.md](references/subagents.md).
...
- Strategist: returns six sections (evidence-linked observations, reusable rules, Ready Queue candidates, stop/continue updates, queue edits, escape/confirmation need). Does not write any session file; apply all returned outputs via `aet.py queue-add` / `queue-drop` and `session.md` edits.
