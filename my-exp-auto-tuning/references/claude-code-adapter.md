Read this file only when the session runtime is `claude`.
...
4. Determine terminal status: `finished`, `failed`, `inconclusive`, or `infra_failed`, classified against [Separating `failed` from `infra_failed`](workflow.md#separating-failed-from-infra_failed). `infra_failed` re-queues the candidate automatically.
...
8. If not, run `aet.py loop-state` and follow its `YOU` block — this is the control-flow router; run it right after `record`, before selecting or launching anything. It counts the planned queue from `results.csv`, computes `projected_ready_after_launch`, names the free GPU-slot occurrences, and routes the Strategist transaction when the projected count is below total_capacity.
9. Execute what loop-state routed. Launch each candidate onto its named GPU-slot occurrence: `aet.py create-run --run-id <id> --gpu-id <gpu>` → launch with `run_in_background=True` → `aet.py record --status running`. A GPU id may appear more than once when stored `max_per_gpu > 1`; each occurrence is one free slot. When loop-state routes a Strategist call because `projected_ready_after_launch < total_capacity`, run the three-beat transaction described in [subagents.md](subagents.md):
   - **Beat 1** `aet.py strategist-begin` snapshots pending runs and finished evidence versions; computes the branch; opens the call; and prints the exact tool call and payload. If a call is already open it refuses.
   - **Beat 2** make the printed tool_use. For a resume, output the `SendMessage` tool call directly targeting the stored id — it always works; do NOT skip it for any reason (anti-pattern #12). If it returns `success:false`, close this resume transaction with the strict pure-cleanup arguments in Beat 3; the next `strategist-begin` routes a fresh spawn. Use `Agent(subagent_type="experiment-strategist", run_in_background=...)` only for a fresh route. Between Beat 2 and Beat 3, keep processing other completion notifications; each `record` adds to pending without disturbing the open call.
   - **Beat 2.5 (mandatory)** When the Strategist subagent returns (completion notification or foreground return), **read its full output** — candidates JSON, observations, reusable rules, stop/continue update, queue edits. Extract the actual candidate count. Do NOT skip this step and immediately close with `--candidates-count 0` (anti-pattern #11).
   - **Beat 3** `aet.py strategist-return --call-id C --candidates-count K [--agent-id A] [--observations-present] [--reusable-rules-present] [--queue-edits-present] [--stop-update-present]` requires non-negative `K` and an id for every fresh invocation. `K` must be the actual candidate count from the Strategist's response — not a number you decided in advance. Run `loop-state`, apply the returned session updates, then re-run `loop-state`. Pure resume cleanup requires `K=0`, no id, and no presence flags.

   The script owns fresh-vs-resume-vs-confirmer, the exhaustion handshake (`pending_exhaustion_confirmation`, promotion of a confirmer to Primary), and quiescence. Quiescence means no `planned`/`created`/`running` rows and no live process matching the stored session process pattern. Do not hand-evaluate it. On `CONFIRMED_EXHAUSTION`, the next steps run in the same turn. The only valid reasons to not begin a call are the self-evaluatable stop conditions and an already-open call; "recently called", "pending is empty", and "no new completions" are not suppression conditions (see anti-patterns #8 and #9 in [../SKILL.md](../SKILL.md)).
...
- Pass the session path, ledger summary, and bounded write scope explicitly because the subagent starts without the current session context.
...
| Strategist subagent | `spawn_agent(task_name="experiment_strategist", message="...", fork_turns="all")`; resume with `send_message` or `followup_task`, then `wait_agent` | `Agent(subagent_type="experiment-strategist")` or `SendMessage` continuation |
