...
TERMINAL_STATUSES = {"finished", "failed", "inconclusive", "superseded", "infra_failed"}

# An infra_failed run died for a reason unrelated to its parameters (OOM, CUDA init,
# missing dependency, full disk, killed process), so it never tested its hypothesis.
# It ends the run and re-queues the candidate, but it never enters the pending set the
# Strategist analyzes — reading it as evidence would fabricate a forbidden region.
EVIDENCE_STATUSES = TERMINAL_STATUSES - {"infra_failed"}
...
def load_loop_state(session: Path) -> dict[str, Any]:
    ...
    for row in read_rows(session / "results.csv"):
        if row.get("status") in EVIDENCE_STATUSES:
            rid = row.get("run_id", "")
...
def query_slots(policy: dict[str, Any], allow_over_cap: bool = False) -> list[dict[str, Any]]:
    ...
    for row in gpu_info:
        ...
        # min_free_memory_mb is the per-job memory requirement, so free memory divides into
        # slots: a card with 6 GB free fits one 5 GB job, not `capacity` of them. Requirements
        # are usually conservative, so the next loop-state re-measures and may open one more.
        if min_free_memory_mb:
            free_slots = min(free_slots, memory_free // min_free_memory_mb)
        ...
        result.append(
            {
                ...
                "capacity": capacity,
                "memory_slots": (memory_free // min_free_memory_mb) if min_free_memory_mb else None,
...
def compute_next(state: dict[str, Any], runtime: str, planned_ids: list[int], free_slots: int, total_capacity: int, free_gpu_ids: list[str] | None = None, session: Path | None = None) -> list[str]:
    ...
    if active:
        ...
        return [
            ...
            (
                ...
                f"--call-id {active['call_id']} --candidates-count 0 --resume-failed`; do not abort it. "
        ...
        if not pending_confirmer and stored_hash and current_hash and stored_hash == current_hash:
            if not actions:
                actions.append(
                    ...
                    "no slot changes). Wait for the next completion."
                )
        else:
...
def compute_evidence_hash(session: Path, free_slots: int) -> str:
    """Hash of session state visible to the Strategist: terminal run versions, planned
    count, running count, and free GPU slots. If unchanged since last Strategist call,
    re-calling cannot produce new information."""
    rows = read_rows(session / "results.csv")
    terminal = sorted(
        (row.get("run_id", ""), row_terminal_version(row))
        for row in rows if row.get("status") in TERMINAL_STATUSES
    )
    planned = sum(1 for r in rows if r.get("status") == "planned")
    running = sum(1 for r in rows if r.get("status") == "running")
    raw = f"{terminal}|{planned}|{running}|{free_slots}"
...
def requeue_infra_failed(session: Path, failed_row: dict[str, str], meta: dict[str, Any]) -> tuple[str, int] | None:
    """Clone an infra_failed run's candidate back into the planned queue so the parameters it
    never got to test are tried again. Returns (new_run_id, attempt) or None when the row carries
    no params to retry. The queue_id lineage Q7 -> Q7r1 -> Q7r2 carries the attempt count."""
    params = failed_row.get("params", "").strip()
    if not params or params == "{}":
        return None
    base_queue_id = failed_row.get("queue_id", "") or f"Q{failed_row.get('run_id', '')}"
    match = re.fullmatch(r"(.*)r(\d+)", base_queue_id)
    if match:
        base_queue_id, attempt = match.group(1), int(match.group(2)) + 1
    else:
        attempt = 1
    rows = read_rows(session / "results.csv")
    run_id = str(next_run_id(session))
    row = {key: "" for key in RESULT_COLUMNS}
    row["run_id"] = run_id
    row["queue_id"] = f"{base_queue_id}r{attempt}"
    row["status"] = "planned"
    row["params"] = params
    for field in ("hypothesis", "expected_signal", "rationale", "priority"):
        row[field] = failed_row.get(field, "")
    row["goal"] = meta.get("goal", "")
    reason = failed_row.get("annotation", "") or "no reason recorded"
    row["annotation"] = f"infra retry #{attempt} of run {failed_row.get('run_id', '')}: {reason}"
    rows.append(row)
...
def _run_has_live_process(output_dir: str) -> bool:
    """Check whether any live process has output_dir in its command line."""
    if not output_dir:
        return False
    try:
    ...
    for line in out.splitlines():
        if output_dir in line and not is_wrapper_line(line):
            return True
    return False
...
def command_record(args: argparse.Namespace) -> None:
    ...
    if args.status in TERMINAL_STATUSES and not args.force:
        check_dir = args.output_dir or row.get("output_dir", "")
        if check_dir and _run_has_live_process(check_dir):
            emit(
                ok=[f"REFUSED: a live process references output_dir {check_dir}"],
                you=[
                    f"The process for run {run_id} is still alive. Kill it first, then re-run this command.",
                    "If this is a false positive (unrelated process), pass `--force` to override.",
                ],
            )
            raise SystemExit(1)
    ...
    if args.status in EVIDENCE_STATUSES:
        version = row_terminal_version(row)
    ...
    ok_lines = []
    if args.status == "infra_failed":
        requeued = requeue_infra_failed(session, row, meta)
        if requeued:
            ok_lines.append(
                f"candidate re-queued as planned run {requeued[0]} "
                f"(queue_id {row.get('queue_id', '')} -> attempt #{requeued[1]}); pending unchanged"
            )
        else:
            ok_lines.append("no params on this row, so nothing was re-queued")
    ...
    pending_note = "results.csv updated" if args.status == "infra_failed" else "results.csv updated; pending += this run"
    emit(
        ok=[f"run {run_id} -> {args.status}{metric_str}", pending_note] + ok_lines,
        state=state_lines,
        you=you,
    )
...
def reconcile_hint(session: Path) -> str | None:
    ...
    msg = (
        ...
        f"--status finished|failed|inconclusive|infra_failed` after parsing its metrics."
    )
    if live == 0:
        msg += (
    ...
    if pattern:
        ...
        if ledger_running > live:
            ...
            blockers.append(
                ...
                f"`aet.py record --run-id <id> --status finished|failed|inconclusive|infra_failed`."
            )
    else:
...
def command_gpu_slots(args: argparse.Namespace) -> None:
    ...
    if args.json:
        ...
        return
    for item in slots:
        mem_slots = "" if item["memory_slots"] is None else f", memory_slots={item['memory_slots']}"
        print(
            ...
            f"capacity={item['capacity']}{mem_slots}, free={item['free_slots']}"
        )
    ...
    if active:
        emit(
            ...
            you=[
                ...
                f"if its output is lost, resume that subagent to re-request it. On a resume-route failure, close with `aet.py strategist-return --call-id {active['call_id']} --candidates-count 0 --resume-failed`; reserve strategist-abort for a fresh call whose spawned subagent is truly unreachable.",
    ...
    if args.resume_failed:
        ...
        if invalid_cleanup:
            emit(
                ...
                you=[
                    "Use candidates-count=0, no agent id, and no presence flags; this call remains open."
                ],
            )
            raise SystemExit(1)
...
def add_policy_flags(parser: argparse.ArgumentParser, *, include_process_pattern: bool = False) -> None:
    ...
    parser.add_argument("--min-free-memory-mb", type=int, help="Per-job GPU memory requirement; slots on a GPU are capped at free_memory // this")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("record", help="Record a status/metric change. Terminal statuses refuse if the run's process is still alive (override with --force).")
    p.add_argument("--session")
    p.add_argument("--project-root", default=".")
    p.add_argument("--run-id", type=int, required=True)
    p.add_argument("--status", choices=["created", "running", "finished", "failed", "inconclusive", "infra_failed", "superseded"], required=True)
    ...
    p.add_argument("--force", action="store_true", help="Skip live-process check for terminal statuses")
    ...
    p = sub.add_parser("strategist-return", help="Close the transaction after the subagent returns: version-clears the pending run snapshot, records agent id, applies the exhaustion handshake, stores an evidence hash for state-unchanged detection, and prints gated YOU obligations.")
    p.add_argument("--session")
    p.add_argument("--project-root", default=".")
    ...
    p.add_argument("--resume-failed", action="store_true", help="Pure cleanup after an actual resume-call failure. Valid only for invocation=resume with candidates-count=0, no agent id, and no presence flags. Preserves pending evidence and makes the next begin fresh-spawn")
    ...
    p = sub.add_parser("status", help="Print session path, objective, run/status counts, and current best.")
    p.add_argument("--project-root", default=".")
    p.add_argument("--session")
    p.add_argument("--goal", choices=["max", "min"])
    p.set_defaults(func=command_summarize)

    p = sub.add_parser("summarize", help="Same as status; accompanies the Session Final Analysis.")
    p.add_argument("--project-root", default=".")
    p.add_argument("--session")
    p.add_argument("--goal", choices=["max", "min"])
    p.set_defaults(func=command_summarize)
