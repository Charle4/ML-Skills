...
| 7 | **Self-declaring exhaustion, convergence, or plateau** — concluding from your own reading of results that the search space is exhausted, a ceiling has been reached, or that further tuning is futile, then stopping or skipping Strategist on that basis | You lack the full search-history perspective; such judgments are highly error-prone. Only the Strategist has authority to declare exhaustion. | Never self-evaluate plateau, convergence, or exhaustion. When Ready Queue is insufficient, call Strategist. Only when Strategist returns 0 candidates and explicitly declares exhaustion should you consider stopping — and even then a second independent Strategist confirmer must agree. Claude Code: fresh-context confirmer `Agent`, not a `SendMessage` resume. Codex: fresh confirmer `spawn_agent`, not a `send_input` resume. See Default Stopping Rules. **Signal**: if you find yourself about to write "this direction is exhausted", "a ceiling has been reached", or "lcdf is a dead end" — that sentence is the trigger to call Strategist, not to stop. |
| 8 | **Skipping Strategist because it was "recently called" or "has no new data"** — reasoning that calling Strategist is pointless because it just ran, or because `runs_since_last_strategist` is empty, so there is nothing new for it to analyze | Recency and empty run lists are not suppression conditions. Strategist always plans from the full session state and can add valuable candidates even immediately after a prior call. The only valid suppression conditions are: explicit user stop, target cleanly met, budget consumed, permission unavailable, or a background Strategist already in flight. | When Ready Queue count < total_capacity, call Strategist — period. **Signal**: if you find yourself thinking "Strategist was just called so I'll skip it" or "there's nothing new for Strategist to analyze" — that reasoning is invalid. Call it. |
| 9 | **Skipping Strategist on keepalive invocations that found no new completions** — running `aet.py status`, seeing no change since last check, and concluding "nothing to do this invocation, continue waiting" without executing the Ready Queue check | The Ready Queue count check is unconditional and independent of whether new completions occurred. `aet.py status` showing no change means step (1) has no action; it does NOT mean step (2) can be skipped. | After `aet.py status` shows no new completions: still check Ready Queue count. If count < total_capacity and no background Strategist is in flight, call Strategist immediately — even with 0 new completions, even with all GPU slots occupied. **Signal**: if you find yourself writing "no new completions, continue waiting" — that sentence is the trigger to check Ready Queue count, not to stop. |
...
9. If Ready Queue count < total_capacity (total_capacity = sum of `capacity` fields across all allowed GPUs from `aet.py gpu-slots --json`; equals capacity_per_gpu × gpu_count when all GPUs share the same capacity setting):
   - If queue is **empty**: call Strategist (blocking); apply returned observations and candidates before launching anything.
   - If queue is **non-empty but below total_capacity**: call Strategist (Claude Code: `Agent(..., run_in_background=True)` or `SendMessage`; Codex: blocking `spawn_agent`/`send_input`). Record the run_ids passed at call time; when Strategist returns, only clear those IDs from `runs_since_last_strategist` — runs completed during analysis accumulate for the next call.
   ...
   - **Claude Code only — continuation over fresh spawn**: if `strategist_agent_id` in plan.md Loop State is not null, resume the existing Strategist via `SendMessage` instead of spawning fresh. Pass only the new `runs_since_last_strategist` and current state; the Strategist retains its session context. **Exception**: if `pending_exhaustion_confirmation` is true, ignore `strategist_agent_id` and spawn a fresh confirmer instead — the exhaustion handshake requires an independent context (see Default Stopping Rules). If SendMessage fails, fall back to fresh spawn and update `strategist_agent_id`. See `references/claude-code-adapter.md` step 9 for the full protocol.
   - **Codex only — continuation over fresh spawn**: if `strategist_agent_id` in plan.md Loop State is not null, resume the existing Strategist via `send_input(target=strategist_agent_id, ...)` instead of spawning fresh, then `wait_agent` when the result is needed. Pass only the new `runs_since_last_strategist` and current state; the Strategist retains its session context. **Exception**: if `pending_exhaustion_confirmation` is true, ignore `strategist_agent_id` and spawn a fresh confirmer instead — the exhaustion handshake requires an independent context. If the target agent is still running, `send_input` queues by default; use `interrupt=true` only when intentionally abandoning its current task. If it was closed, use `resume_agent(id=...)` before sending when continuity is useful. If resume/send fails, fall back to fresh spawn and update `strategist_agent_id`.
...
- `Ready Queue`: launchable candidates without assigned run ids yet. Keep this count at or above total_capacity whenever useful unexplored regions remain. total_capacity = configured capacity_per_gpu × number_of_gpus (constant; read from `aet.py gpu-slots`). Call Strategist whenever count drops below total_capacity, ensuring the queue is always pre-stocked for upcoming slots.
...
| ------- | ---- | -------------------- |
| `Ready Queue` count < total_capacity (constant: capacity_per_gpu × gpu_count from `aet.py gpu-slots`) | Strategist — blocking if queue empty, background (`run_in_background=True` Agent, Claude Code only) if queue non-empty. **Claude Code**: prefer `SendMessage` continuation over fresh spawn when `strategist_agent_id` is set in plan.md Loop State, except when `pending_exhaustion_confirmation` is true (then force a fresh confirmer spawn). **Codex**: prefer `send_input(target=strategist_agent_id, ...)` continuation over fresh spawn when `strategist_agent_id` is set, except when `pending_exhaustion_confirmation` is true (then force a fresh confirmer spawn) | Append returned `observations_to_append` to `observations.md`; clear only the `runs_since_last_strategist` entries passed at call time; append returned rows to `plan.md`; update Stop/Continue Rule; launch ready rows; record returned agent id to `strategist_agent_id` on fresh spawn |
...
- Strategist calling follows one rule throughout: Ready Queue count < total_capacity → call Strategist. total_capacity is constant (capacity_per_gpu × gpu_count). When queue is empty, the call is blocking; when queue is non-empty, the call is background where the runtime supports it (Claude Code) so experiments continue. This rule never changes. When a Strategist returns zero candidates, the Queue remains empty (0 < total_capacity), so every subsequent trigger keeps invoking the Strategist with the standard neutral prompt and complete session state. **Claude Code**: "invoking" normally means a `SendMessage` resume of the Primary, except during exhaustion confirmation — when `pending_exhaustion_confirmation` is set, it means a fresh confirmer spawn (see Default Stopping Rules and `references/claude-code-adapter.md` step 9). **Codex**: "invoking" normally means a `send_input` resume of the Primary, except during exhaustion confirmation — when `pending_exhaustion_confirmation` is set, it means a fresh confirmer spawn (see `references/subagents.md`).
...
- Stop condition for plateau/exhaustion: two exhaustion verdicts from **independent contexts**, both produced while fully quiescent (no experiments running, Ready Queue empty), both returning zero candidates and declaring exhaustion. The two verdicts must not share a context: (1) the verdict that opens the handshake, and (2) a fresh confirmer spawned specifically to check it.
  - **Claude Code**: verdict (1) is the continuous-context (Primary) Strategist resumed via `SendMessage`; verdict (2) MUST be a fresh `Agent` spawn — never a `SendMessage` resume of the Primary, because re-messaging the context that produced the verdict gives no independence. If the fresh confirmer returns candidates instead of agreeing, the Primary's verdict is overturned: append the candidates, continue the loop, and **promote the confirmer to Primary** (`strategist_agent_id` ← the confirmer's `agentId`; the old Primary is no longer addressed). The next exhaustion handshake then starts from this new Primary.
  - **Codex**: verdict (1) is the continuous-context (Primary) Strategist resumed via `send_input`; verdict (2) MUST be a fresh `spawn_agent` confirmer — never a `send_input` resume of the Primary. If the fresh confirmer returns candidates instead of agreeing, the Primary's verdict is overturned: append the candidates, continue the loop, and **promote the confirmer to Primary** (`strategist_agent_id` ← the confirmer's agent id; the old Primary is no longer addressed). The next exhaustion handshake then starts from this new Primary.
  - Any Strategist that returns candidates anywhere discards all pending exhaustion declarations and resets the handshake to zero. See `references/claude-code-adapter.md` step 9 for the full Claude Code handshake.
...
- **Codex**: Your active instructions identify you as "Codex, a coding agent based on GPT-5". Skill installed at `~/.codex/skills/my-auto-experiment-tuning/`. Shell sessions are the primary experiment execution path; no `run_in_background`, no background notifications, no `/loop`. Codex subagents can be resumed with `send_input(target=agent_id, ...)` when you preserve `strategist_agent_id`.
...
5. Fill Ready Queue before launching:
   **If `Ready Queue` count < total_capacity, call Strategist before selecting any candidates.** This is the same invariant as the steady-state cycle — it applies at session start too. Strategist plans candidates from `plan.md`'s objective, hypotheses, and coupled parameters; it does not require completed runs to do so. Do not manually design the initial parameter set yourself; that is Strategist's responsibility.

   Use the runtime-appropriate syntax from `references/subagents.md`. At session start the queue will typically be well below total_capacity, so the call is blocking (queue empty → blocking per the standard rule in subagents.md). Because all slots are free at startup, expect Strategist to return at least 2× total_capacity (current_free_slots + total_capacity): if it returns only total_capacity, launching the first wave empties the queue and forces an immediate redundant blocking re-call against the same empty result state. With ≥ 2× total_capacity, the queue still holds ≥ total_capacity after the first launch wave.
...
**Claude Code**: spawn via the `Agent` tool using the registered subagent type `experiment-strategist`. Use Claude Code syntax such as `Agent(subagent_type="experiment-strategist", prompt="...")`. Pass only session-specific context in the `prompt` parameter; the subagent's role instructions are already loaded from its own system prompt. See `references/subagents.md` for the prompt template.

**Codex**: spawn the first Primary with the registered custom agent selected as `experiment-strategist`. Use Codex syntax such as `spawn_agent(message="...", fork_context=true)` when current main-thread context must be inherited; omit `fork_context` when the prompt plus durable files are sufficient. Pass only session-specific context in the `message` parameter; the subagent's role instructions are already loaded from its registered Codex custom agent file. Save the returned agent id as `strategist_agent_id`. Resume that same Primary with `send_input(target=strategist_agent_id, message="...")`, then `wait_agent` when the result is needed; this reuses the subagent context instead of creating a clean context. If the subagent is still running, `send_input` queues by default; use `interrupt=true` only when intentionally replacing the current task. If the agent has been closed, use `resume_agent(id=...)` before sending when continuity is still useful. Use `close_agent` only when you are deliberately ending that reusable context. `multi_tool_use.parallel` is for parallel tool calls, not subagent creation.

If delegation is not authorized, perform strategy and planning inline yourself.
