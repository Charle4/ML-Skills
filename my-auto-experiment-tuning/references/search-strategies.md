...
- Change one conceptual factor per batch unless testing an interaction.
...
- For open-ended tuning, assume the search should continue across many batches. A good result should trigger refinement, boundary expansion, seed checks, or alternative-regime tests rather than an immediate final answer.
...
- Use small factorial or fractional-factorial batches to expose interactions when the space is not yet understood.
- If two one-dimensional scans disagree, assume a hidden interaction until ruled out.
- If a local optimum is found by coordinate descent, run at least one broader escape batch around alternative regimes before declaring the space exhausted.
...
- Mark batches that used a later-discovered data conversion or metric bug as `inconclusive`, then rerun only the informative candidates cleanly.
...
- If the user expects hundreds of experiments, cycle phases: broad interaction design → local refinement → clean confirmation → escape batch → next interaction family. Do not spend the whole session in one local basin.
...
A good batch has:
- 3-12 independent configurations, depending on runtime and GPU slots
...
- Plan a rolling queue at least one batch ahead so GPUs do not idle after collection.
- Keep batch sizes near available slot count unless the method's contention rules forbid it.
...
- Record every batch even when it is a failure; failed regions are what justify continued expansion rather than repeated local probing.
...
- at least one broad interaction batch after implementation fidelity is trusted
- at least one local refinement batch around the current best
- at least one escape batch testing a different regime after the current best
