...
9. Execute what loop-state routed. Launch each candidate onto its named GPU-slot occurrence: `aet.py create-run --run-id <id> --gpu-id <gpu>` → launch with `run_in_background=True` → `aet.py record --status running`. A GPU id may appear more than once when stored `max_per_gpu > 1`; each occurrence is one free slot. When loop-state routes a Strategist call because `projected_ready_after_launch < total_capacity`, run the three-beat transaction described in [subagents.md](subagents.md):
   - **Beat 1** `aet.py strategist-begin` snapshots pending runs, finished evidence versions, and pending benchmark marker versions; computes the branch; opens the call; and prints the exact tool call and payload. If a call is already open it refuses.
   - **Beat 2** make the printed tool_use. For a resume, call the literal `SendMessage` tool targeting the stored id. If it returns `success:false`, close this resume transaction with the strict pure-cleanup arguments in Beat 3; the next `strategist-begin` routes a fresh spawn. Use `Agent(subagent_type="experiment-strategist", run_in_background=...)` only for a fresh route. Between Beat 2 and Beat 3, keep processing other completion notifications; each `record` adds to pending without disturbing the open call.
   - **Beat 3** `aet.py strategist-return --call-id C --candidates-count K --benchmark-promotion-run-ids none|IDS --benchmark-reviewed-run-ids none|IDS [--agent-id A] [--observations-present] [--reusable-rules-present] [--queue-edits-present] [--stop-update-present]` requires non-negative `K`, both selectors, and an id for every fresh invocation. Promotion creates durable debt with approved evidence version and metric/artifact locators. Run `loop-state`, apply the listed project-table updates, acknowledge successful ids with `aet.py benchmark-ack`, then re-run `loop-state`. Ack rejection requeues changed finished evidence for review. Pure resume cleanup requires `K=0`, both selectors `none`, no id, and no presence flags.
