...
- `gpu_id` for each run.
- `output_dir`, already created by `aet.py create-run` (the `output_dir:` line in its output).
...
- The parent agent has already called `create-run`; verify `run_dir/output` exists, then launch and record running.
...
Return the run id, command, GPU id, output directory, log path, current process/session status, and whether `record --status running` was completed.
...
1. Verify `record --status running` completed for each run
