...
- Keep `Ready Queue` candidates strictly greater than `current_free_slots` whenever useful search remains; usually return no more than about 2x configured capacity unless runs are very short.
...
4. Check free GPU slots: `aet.py gpu-slots`
5. For each free slot, take the top-priority Ready Queue row:
   a. Create unique output dir: `aet.py unique-dir RESULTS_BASE/run_name --mkdir`
   b. Register: `aet.py create-run --session SESSION --run-id N --name ... --params '...' --gpu-id G --output-dir DIR --log-path DIR/train.log`
   c. Launch using the runtime-appropriate method:
      - Claude Code: `Bash(command="python -u SCRIPT --gpu_id G --output_dir DIR > DIR/train.log 2>&1", run_in_background=True)`
   ...
   d. Record running: `aet.py record --session SESSION --run-id N --status running`
   e. Move row from Ready Queue to Running in plan.md
