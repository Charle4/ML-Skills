...
5. Record terminal status and metrics in the session ledger using `aet.py record`; include concise notes only after trust status is known.
...
Do not delete failed outputs. Mark contaminated results as inconclusive. Add detailed trajectory/trust-check edits to `runs/<id>/summary.md` after the terminal `record`, because later `record` calls rewrite that file.
